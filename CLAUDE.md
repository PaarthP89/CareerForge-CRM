...
- [2026-07-02] Phase 5.2 complete (pending human review): Built `/app/api/resume/match` as the API/pipeline-only layer (no UI, per anti-goals). Installed `@google/generative-ai` (approved this session). Added migration `20260702000000_add_job_matches_table.sql` (new `job_matches` table + RLS, applied to remote — see 2.9 proposal above) and `JobMatch` type in `types/index.ts`. Added `lib/gemini.ts`: a lazy-singleton `GoogleGenerativeAI` client reading `GEMINI_API_KEY` (throws clearly if unset, checked explicitly at the top of the route so a missing key returns a clean 500 instead of an uncaught exception), plus `generateText`/`parseJsonResponse` helpers (the latter strips ` ```json` fences and throws a typed `GeminiJsonParseError` on malformed output, caught per-call by the route). Considered adding the `server-only` npm package to guard `lib/gemini.ts` against Client Component imports per the builder prompt's intent, but it isn't in `package.json` and wasn't part of the one dependency approved this session (`@google/generative-ai` only) — skipped installing it; the file is only ever imported from `app/api/resume/match/route.ts`, a server-only route file by construction, so the guard is enforced structurally instead. `route.ts` POST handler: fetches the caller's `resumes.content` (400 if empty/whitespace-only, no Gemini call made), fetches active (`deleted_at is null`) jobs (200 with a zeroed summary and no Gemini calls if there are none), deduplicates by case/whitespace-normalized title into a title→job-ids map, runs a single Stage-1 Gemini pre-filter call (strict-JSON `{irrelevant_titles: string[]}`, degrades to "keep everything" on parse failure rather than aborting), then chunks survivors into batches of 75 titles and scores them via Stage-2 Gemini calls with bounded concurrency (max 3 in flight, via a shared-queue worker pool) — each batch upserts its `job_matches` rows immediately on success (`onConflict: 'user_id,job_id'`, one row per underlying job id even though scoring is per unique title) and increments a `failedBatches` counter without aborting the run on a batch-level network/parse/upsert error. Returns `{ totalJobs, prefiltered, scored, failedBatches }`. GET handler returns the caller's `job_matches` rows joined with `jobs(title, company)`, ordered by score descending — this is the only way to inspect results until 5.3 builds a UI. `npm run lint` and `npm run build` both pass clean; `npm test`/`npm run test:e2e` still have no script (pre-existing gap tracked since Phase 1.1, not introduced here). Verified via curl against the running dev server: unauthenticated `GET`/`POST /api/resume/match` both correctly return 401. No `GEMINI_API_KEY` is currently set in `.env.local` — added to the known-issues/notes below rather than blocking, consistent with how prior subphases handed off credential gaps. No `.env.example` file exists in the repo to update (checked — none present, so this step was a no-op). No changes to `/app/resume/page.tsx`, `components/resume/resume-editor.tsx`, `/app/dashboard`, or `/workers/scraper` — all consistent with the anti-goals for this subphase. | Known issues: (1) `GEMINI_API_KEY` is not yet in `.env.local` or GitHub Actions secrets — the route's explicit env check will return a clean 500 until it's added, but the actual scoring pipeline (Stage 1 + Stage 2 against real Gemini calls) has not been exercised end-to-end with a live key. (2) Like 5.1, the authenticated happy path (real resume + real job corpus → pre-filter → batch scoring → persisted `job_matches` rows) has not been click-tested in a real browser/curl-with-session since I don't hold login credentials — only the 401 guard was verified live. | Notes for Phase 5.3: `GET /api/resume/match` returns `{ matches: [{ id, job_id, score, reasoning, matched_at, jobs: { title, company } }] }` sorted by score descending — that's the exact shape 5.3's UI should render (e.g. a sorted/filterable match list on the dashboard or resume workspace). 5.3 should also decide what "missing-term suggestions" means given `jobs` still only has title/company (no JD text) — likely scoped to resume-side suggestions only, not per-job term diffing, unless a JD-fetching subphase lands first.

- [2026-07-02] Section 3 checkbox for 5.2 checked (human gave explicit permission this session to make the Section 3 edit directly, same precedent as 5.1). Human has decided to proceed straight to 5.3 and treat Phase 5 as code-complete now, deferring the actual `GEMINI_API_KEY` setup to a later session ("you'll see later what I mean by keys" — meaning currently unspecified, possibly more than one key/rotation scheme). This means the 5.2 pipeline's live Gemini calls (pre-filter + batch scoring against a real key) remain unverified end-to-end; that verification did not happen here and should not be assumed done just because the box is checked. Proceeding to build 5.3 next in this same session.

- [2026-07-02] Phase 5.3 complete (pending human review), scope redefined through discussion with the human — see the 2.9 proposal directly above for the technical reasoning on why the original "missing-term suggestions" line couldn't be built as literally written (no JD text exists on `jobs`). Landed as two features:
  1. **Per-job live compare**: `POST /app/api/resume/match/[jobId]/route.ts` (new) — ownership-scoped job + resume fetch, then a plain `fetch()` (10s timeout via `AbortController`, custom User-Agent) against `jobs.url`, HTML→text extraction via new `lib/html.ts` (`extractReadableText`: strips `<script>`/`<style>`/comments/tags, decodes basic entities, collapses whitespace, caps at 20k chars; `isViableJobDescription`: rejects anything under 200 chars as a probable JS-walled/blocked page). If extraction succeeds, prompts Gemini with the real resume + JD text for `{score, reasoning, missing_keywords}`; if it fails, falls back to a title/company-only prompt with `missing_keywords: []` and `jd_fetched: false`. Result is validated (`isValidCompareResult`) and upserted into `job_matches` (`onConflict: 'user_id,job_id'`) alongside the new `missing_keywords`/`jd_fetched` columns, then returned to the client. A malformed/failed Gemini response returns a clean `502` rather than persisting garbage.
  2. **Bulk match list**: new `/app/matches` page (`app/matches/page.tsx` + `components/matches/matches-list.tsx`) reading the existing 5.2 `job_matches` table server-side (joined with `jobs(title, company)`, ordered by score), with a "Run Match" button that re-triggers the bulk `POST /api/resume/match` pipeline from 5.2 and `router.refresh()`s on completion; each row links out to the per-job live-compare flow.
  On the trigger side: `components/dashboard/jobs-table.tsx` gained a `GitCompare`-icon link per active row (`/resume?jobId=<id>`, plain `<a>` so it's a full navigation, not client-side routing — deliberate, since `app/resume/page.tsx` needs to re-run as a Server Component to read the new `?jobId=` search param and fetch that job's title/company for the panel header). `app/resume/page.tsx` now accepts `searchParams: Promise<{ jobId?: string }>` (Next 16 async searchParams) and, when present and owned by the caller, renders the new `components/resume/job-compare-panel.tsx` (Client Component) above the editor. That panel auto-fires the compare `POST` on mount (`useEffect` → `performCompare`, restructured specifically to keep all `setState` calls inside the async `.then`/`.catch` rather than synchronously in the effect body, to satisfy the `react-hooks/set-state-in-effect` lint rule that failed the first pass), shows a `Loader2` "Running comparison…" state, then renders the score (color-coded `Badge`: green ≥70, gray ≥40, red below), reasoning, and missing-keyword chips — or a Retry button on error. Added `components/nav-links.tsx` (plain `<a>` tags, no client interactivity needed) and wired it into the header of `/dashboard`, `/resume`, and `/matches` for cross-navigation between the three pages. `npm run lint` and `npm run build` both pass clean (one lint failure surfaced mid-build on the effect/setState pattern above, fixed by moving all state mutation inside the promise chain and dropping the ref-based cleanup in favor of a `useCallback`-memoized fetch function); one TypeScript build failure surfaced in `app/matches/page.tsx` (Supabase's FK-join inference typed `jobs` as an array `{title,company}[]` rather than a single object since no generated `Database` types are wired up — fixed by mapping `row.jobs` down to `row.jobs[0] ?? null` server-side before passing to the client component). `npm test`/`npm run test:e2e` still have no script (pre-existing gap, not introduced here). Verified via curl against the running dev server: `GET /matches`, `GET /resume`, and `GET /resume?jobId=<fake>` all return `200` (render their sign-in/empty shells per the existing unauthenticated pattern), `POST /api/resume/match/<fake-id>` correctly returns `401` unauthenticated. Migration `20260702120000_add_job_matches_live_compare_columns.sql` applied to remote, confirmed via `supabase migration list`. Section 3 checkbox for 5.3 checked directly (same standing permission as 5.1/5.2 this session). | Known issues: (1) Same as 5.2 — `GEMINI_API_KEY` still isn't set in `.env.local`, so neither the bulk nor the per-job live-compare pipeline has been exercised against a real Gemini response yet; only the 401/200 shell-rendering guards were verified live. (2) The live-fetch reliability question flagged during scoping (JS-rendered/bot-walled job boards returning unusable HTML) is architecturally handled (graceful title-only fallback) but not yet empirically tested against the real ~3,118-job corpus's actual mix of ATS platforms — worth a spot-check once a key is in place, to see how often `jd_fetched` ends up `false` in practice. (3) No UI regression-tested in a real browser (no login credentials held), consistent with every prior subphase's handoff. | Notes for whoever picks this up next: once `GEMINI_API_KEY` is added, the two flows to manually verify end-to-end are (a) click "Run Match" on `/matches` and confirm the ranked list populates, and (b) click the compare icon on a `/dashboard` row and confirm `/resume?jobId=...` auto-runs and shows a score — try at least one job whose `url` is a Greenhouse/Lever-style posting (should get `jd_fetched: true`) and one that's likely JS-walled (e.g. LinkedIn) to confirm the fallback path actually degrades gracefully rather than erroring.

- [2026-07-02] Backlog / notes for future development, raised by the human after reviewing Phase 5 — not implemented in this session, captured here for whoever picks these up next:

  **1. What's actually left from Phases 1-5:** every Section 3 checkbox through 5.3 is now checked, but "checked" only means code-complete, not verified. Concretely still open: (a) `GEMINI_API_KEY` has never been set, so the entire Gemini pipeline (5.2 bulk scoring, 5.3 live compare) has zero real-world runs — only auth/shell guards were curl-verified; (b) no Phase 5 UI (5.1 editor, 5.2/5.3 match flows) has been click-tested in a real logged-in browser session, for the same "no login credentials" reason noted on every prior subphase; (c) the live-JD-fetch reliability question (how often real postings resolve to usable text vs. a JS-walled/blocked page) is untested against the actual ~3,118-job corpus; (d) from Phase 4b, there's still no permanent-delete/empty-trash action for the Trash tab (soft-deleted rows persist indefinitely) — never in scope, just still true. There is no drafted Phase 6 yet.

  **2. Scraped `jobs.url` values are not always the actual application link.** The human flagged that some listings' `url` lands on something other than where you'd actually apply — likely an aggregator's own listing/redirect page (from `workers/scraper/sources/speedyapply.ts` or `vanshb03.ts`, which extract links out of README/table markup rather than the posting's own "Apply" URL) rather than the company's/ATS's real application form. Needs investigation in `/workers/scraper` specifically: (i) whether the aggregator sources expose a more specific per-posting/ATS-direct link than what's currently captured by `extractUrl` in `workers/scraper/lib/markdown.ts`, and if not, (ii) whether resolving redirects (follow the current `url` server-side and capture the final landing URL) gets closer to the true apply link. This is scraper-side work — per Section 1.4 Rule 2, it must stay inside `/workers/scraper` and not reach into `/app`.

  **3. Dead/expired job listings ("not available", "not found", "position filled", etc.) should never make it into the `jobs` table**, let alone stay there. The human's ask is for these to be detected and removed automatically, ideally *before insertion* — i.e. during the scrape run itself, not as a later cleanup pass a user has to trigger. Likely shape: at scrape time, for each candidate listing, fetch `url` and run a cheap classifier (keyword heuristics on the fetched text like "no longer accepting applications", "position has been filled", "404"/"not found", or a lightweight Gemini call if heuristics prove unreliable) — if it classifies as dead, skip the insert entirely rather than writing then deleting. Two things worth deciding before building this: (a) fetching every candidate URL synchronously during a nightly scrape of ~3,100 listings is a very different cost/latency profile than the on-demand single-URL fetch built in 5.3 — needs its own concurrency/rate-limiting design, not a copy-paste of `lib/html.ts`; (b) per the architectural boundary (Rule 2), this logic and any HTML-extraction helper it needs must live under `/workers/scraper` (e.g. its own `lib/` module, separate from the app-layer `lib/html.ts` built for 5.3), not imported from `/app`.

  The human indicated more items like this are likely to follow ("etc.") — treat this as a growing backlog, not a closed list.
