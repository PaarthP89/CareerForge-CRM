...
- **Watch-item, not yet investigated:** `fetchFailures` rate jumped to ~43% (130/300) on the batch immediately following the three validation runs above (which ran 18-26%). Single data point, no crash (exit 0, `updateErrors: 0`) — worth checking if it persists on the next few runs, since a sustained jump could indicate either a queue-ordering artifact (older-`last_checked_at` rows skewing toward stale/bot-walled domains) or a real regression.
