...
**Filed 2026-08-21, resolved same day:** an eligibility-filtering feature was found already built and applied to the live corpus, but undocumented and uncommitted — full mechanics logged here originally; now tracked as `Phase 8` in §3 after human review. Decisions made: (1) product is single-user permanently (confirmed by the human 2026-08-21) — the hardcoded candidate profile in `eligibility.ts` is intentional, not a gap; no configurability work needed. (2) Formalized as `Phase 8` in §3 (8.1 eligibility feature, 8.2 the Groq model swap) rather than left pending here — lint/152 tests/build all re-verified green before checking those boxes. (3) The `workers/shared/lib/groq.ts` `MODEL_NAME` swap (`llama-3.1-8b-instant` → `openai/gpt-oss-20b`) was split out as its own independent line item (`8.2`) per human direction, rather than staying silently bundled inside the eligibility diff — still not backed by a before/after test in this repo, so treat as a housekeeping change riding on the same commit, not a validated fix. Schema/type promotion into §2.1's canonical block and §2.5's dependency table still needs a human pass (Claude cannot edit those sections directly per §0's permission table) — `jobs.eligibility`/`eligibility_reason` and the matching `types/index.ts` fields are ready to promote whenever that happens.
...
### Phase 8: Eligibility Filtering & Groq Model Housekeeping
*(Built and already applied to the live corpus before this entry existed; retroactively added to the tracker 2026-08-21 after human review of the §2.9 proposal — approved same day. Single-user product, confirmed 2026-08-21: there is and will only ever be one user, so the candidate profile below is intentionally hardcoded, not a config value.)*
- [x] 8.1 Add `jobs.eligibility`/`jobs.eligibility_reason` columns (migration `20260817000000_add_jobs_eligibility.sql`); build `workers/shared/lib/eligibility.ts` — three-layer classifier (title regex → JD-text regex → LLM fallback, same philosophy as `url-quality.ts`/`dead-listing.ts`) for a single hardcoded candidate profile (Bachelor's grad Dec 2026, internship experience only, no full-time work history). Wired into `workers/scraper/index.ts` (skips ineligible listings at insert time, counted as `ineligibleSkipped`) and `workers/link-health/index.ts` (re-classifies tracked jobs on the weekly sweep, soft-deletes newly-ineligible rows, counted as `ineligibleFlagged`).
- [x] 8.2 Independent housekeeping, unrelated to 8.1 — swap `workers/shared/lib/groq.ts`'s `MODEL_NAME` from `llama-3.1-8b-instant` to `openai/gpt-oss-20b`, addressing the JSON-truncation reliability problem already described in that file's own comment. Called out as its own line item per human direction rather than left bundled silently inside the eligibility diff.
...
**Session (2026-08-13): fixed the 2026-08-09 `updateErrors` anomaly, confirmed root cause was the Supabase update path specifically (not the JD-fetch path).** Ran `npm run scraper:dev` clean — 4,648 attempted across all 4 sources, 0 errors, 60 dead-skipped. Found a fix already sitting uncommitted in the working tree from a prior, unlogged session, targeting exactly the 2026-08-09 anomaly: `updateJobWithRetry()` in `workers/link-health/index.ts` wraps every `supabase.from('jobs').update(...)` call (both the soft-delete-on-dead path and the normal url_quality/last_checked_at path) in a 3-attempt retry with a 1s delay between attempts, since postgrest-js only auto-retries GET/HEAD/OPTIONS — a `.update()` is a PATCH and previously had zero built-in retry/backoff, so any transient network blip during a run (heavy concurrent Chromium + fetch traffic) surfaced as an immediate, unretried `TypeError: fetch failed`. Validated the fix by running link-health **three consecutive times** (900 jobs total): `updateErrors: 0` every time (`deadFlagged`/`fetchFailures` per run: 51/61, 30/79, 27/54). Confirmed this was genuinely the fix (not just this session's network being unusually healthy) by reading `updateJobWithRetry` — it targets exactly the per-row update failure mode the 2026-08-09 entry described. `fetchFailures` (the JD-text fetch path, `fetchTextWithTimeout` in `workers/shared/lib/html.ts`) remains nonzero at a ~18-26% rate across all three runs — investigated and confirmed this is **not** the same bug class: `fetchTextWithTimeout`/`fetchWithBrowser` catch every fetch/DNS/timeout/Playwright error internally and classify it (`dns`/`timeout`/`other`) rather than throwing, so a nonzero count here just means real-world pages (bot walls even the Chromium fallback can't beat, genuinely dead links, slow sites hitting the 10s timeout) failed to yield viable text — expected baseline noise for a multi-thousand-domain corpus, not a crash-class bug. `npm test` (152 tests) and `npm run build` both pass clean; lint clean. Committed the retry-logic fix (previously uncommitted) along with this changelog entry. Pushed to `origin/dev` (`593c547`).

**Same session, follow-up run post-push:** ran `link-health:dev` once more against the next 300-row batch to confirm the pushed fix holds on top of `origin/dev`: `checked: 300, deadFlagged: 8, fetchFailures: 130, updateErrors: 0` (exit 0). `updateErrors` still 0, confirming the fix. `fetchFailures` jumped to 130/300 (~43%) — notably higher than the 18-26% seen in the three validation runs above. Not investigated further this session (single data point, exit 0, no crash); noted below as a watch-item since batches are processed oldest-`last_checked_at`-first, so this could just mean the next slice of the queue happens to be denser with bot-walled/stale domains, or could be a real regression worth a closer look if the elevated rate persists on subsequent runs.

**Session (2026-08-15): routine scraper + link-health run.** `npm run scraper:dev` clean — 4,791 attempted across all 4 sources, 0 errors, 54 dead-skipped. `npm run link-health:dev`: `checked: 300, deadFlagged: 23, fetchFailures: 99, updateErrors: 0` (exit 0). `updateErrors: 0` again confirms the 2026-08-13 retry fix continues to hold. `fetchFailures` at 99/300 (~33%) lands between the two prior data points (18-26% validation runs vs. 43% follow-up run), which is more consistent with the queue-ordering hypothesis (rate varies with which slice of stale/bot-walled domains is up next) than a regression — no further action taken.

**Session (2026-08-15), second run same day:** another routine scraper + link-health pass. `npm run scraper:dev` clean — 4,808 attempted, 0 errors, 52 dead-skipped. `npm run link-health:dev`: `checked: 300, deadFlagged: 46, fetchFailures: 82, updateErrors: 0` (exit 0). `fetchFailures` at ~27% sits comfortably inside the 18-43% range already logged above — further evidence for the queue-ordering explanation, not a regression.

**Session (2026-08-21): ran scraper + link-health, discovered an undocumented eligibility-filtering feature already sitting live and uncommitted in the working tree; human reviewed and formalized it same day.** Neither worker's code was touched this session — the ingestion run itself was routine, not a coding subphase. `npm run scraper:dev`: 4,788 attempted across all 4 sources, 0 errors, 52 dead-skipped, **189 newly ineligible-skipped** (never inserted). `npm run link-health:dev`: `checked: 300, deadFlagged: 34, ineligibleFlagged: 138, fetchFailures: 75, updateErrors: 0` (exit 0) — `updateErrors: 0` confirms the 2026-08-13 retry fix still holds; `ineligibleFlagged` rows were soft-deleted (`deleted_at` set), same trash-tab path as dead listings. Both runs completed cleanly with no crashes. Net effect on the corpus: a large one-time wave of previously-tracked jobs aimed at experienced/grad-only candidates is now being filtered out or trashed going forward.

Follow-up same day: reviewed the found feature with the human. Confirmed single-user permanently, so the hardcoded candidate profile in `eligibility.ts` needs no configurability work. Formalized the feature as `Phase 8` in §3 (retroactively, same pattern as Phase 6) — split into `8.1` (eligibility classifier + schema + worker wiring) and `8.2` (the unrelated Groq `MODEL_NAME` swap to `openai/gpt-oss-20b`, called out as its own item rather than left silently bundled). Re-ran lint/`npm test` (152 passing)/`npm run build` clean before checking those boxes. Staged (not committed) all the touched files: `CLAUDE.md`, the new migration and `eligibility.ts`, `types/index.ts`, `workers/link-health/index.ts`, `workers/scraper/index.ts`, `workers/shared/lib/groq.ts`. Still open: schema/type promotion into §2.1/§2.5's canonical blocks needs a human pass (outside Claude's edit permissions), and the Groq model swap still has no before/after test confirming it actually fixes the JSON-truncation issue it's presumed to address.

**Session (2026-08-22): routine scraper + link-health run.** `npm run scraper:dev` clean — 4,790 attempted across all 4 sources, 0 errors, 54 dead-skipped, 198 ineligible-skipped. `npm run link-health:dev`: `checked: 300, deadFlagged: 38, ineligibleFlagged: 109, fetchFailures: 70, updateErrors: 0` (exit 0). `updateErrors: 0` again confirms the 2026-08-13 retry fix continues to hold. `fetchFailures` at ~23% sits comfortably inside the already-established 18-43% range — no action taken.

**Session (2026-08-23): routine scraper + link-health run.** `npm run scraper:dev` clean — 4,789 attempted across all 4 sources, 0 errors, 54 dead-skipped, 180 ineligible-skipped. `npm run link-health:dev`: `checked: 300, deadFlagged: 30, ineligibleFlagged: 105, fetchFailures: 84, updateErrors: 0` (exit 0). `updateErrors: 0` again confirms the 2026-08-13 retry fix continues to hold. `fetchFailures` at 28% sits comfortably inside the already-established 18-43% range — no action taken.

**Open items / known gaps, current as of this entry:**
- `jobs.eligibility`/`eligibility_reason` (Phase 8) need manual promotion into §2.1's canonical schema block and §2.9 cleared — human-only edit, still pending.
- The `openai/gpt-oss-20b` Groq model swap (Phase 8.2) has not been validated with a before/after test in this repo — worth confirming it actually resolves the JSON-truncation problem before relying on it long-term.
- **Watch-item, softening:** `fetchFailures` rate has now been observed at 18-26% (three runs), 43% (one run), 33%, 27% (both 2026-08-15), and 23% (2026-08-22) — the spread looks like normal variance tied to which domains sit next in the oldest-`last_checked_at`-first queue, not a regression. No `updateErrors` in any of these runs. Downgrading urgency; revisit only if a run shows actual `updateErrors` again or `fetchFailures` trends persistently upward.
