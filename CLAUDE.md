...
- [2026-07-07] [Ad hoc, link-health re-check]: Added a second LLM provider path to `workers/shared/lib` — a new `groq.ts` (plain `fetch` against Groq's OpenAI-compatible `chat/completions` endpoint, model `llama-3.1-8b-instant`, **no new npm dependency**) that round-robins across a comma-separated `GROQ_API_KEYS` env var so one key's per-minute/per-day cap doesn't stall a run; `llm.ts`'s `generateText()` now prefers Groq whenever `GROQ_API_KEYS` is set, falling back to the existing Gemini path otherwise. The shared `GeminiRateLimitError` class was renamed to `LlmRateLimitError` (now takes an optional `provider` label, default `'Gemini'`) since it's thrown by both providers on exhaustion and callers (`dead-listing.ts`, `url-quality.ts`) only ever did `instanceof` checks — no behavior change there, just accurate log messages. Reason: re-running `link-health` against the 544 jobs sitting at `url_quality` `generic`/`unknown` hit Gemini's free-tier **daily** quota partway through (first transient 503s, then a real 429 `PerDay` exhaustion), which degraded 56 previously-`generic` rows down to `unknown` since the retry attempt failed rather than being skipped — no data was lost (still reversible, nothing hard-deleted) but nothing was resolved either. Human is provisioning 5-10 Groq keys (6k TPM / 500k TPD each per Groq's docs) specifically to push through that backlog same-day instead of waiting on Gemini's reset. **Needs a new secret**: `GROQ_API_KEYS` (comma-separated) in `.env.local`, not yet added by the human as of this entry — recommend Section 2.3 gain a row for it once promoted. Not yet run against real keys or the actual unknown/generic backlog; `npm run lint` and both worker `tsc --noEmit` are clean (pre-existing `import.meta`/CommonJS errors in `text-classifier.ts` and `parsers.test.ts` are unrelated, predate this change).

- [2026-07-14] [Ad hoc, test infrastructure]: Installed the Vitest stack that §2.6 already committed to on paper but that was never actually present: `vitest`, `@vitejs/plugin-react` (pinned to `4.7.0`, not latest `6.x`, since `6.x` pulls in a `@rolldown/plugin-babel` optional peer that conflicts with `shadcn`'s own `@babel/core@7` peer chain — `4.7.0` avoids that entirely), `jsdom`, `@testing-library/react`, `@testing-library/jest-dom`, `@testing-library/user-event`, `@vitest/coverage-v8`. Human explicitly approved installing these (per Rule 6) after being asked to scope the work. New `vitest.config.ts` at the repo root (jsdom environment by default, `@` path alias mirroring `tsconfig.json`, `tests/unit/setup.ts` as a global setup file). New `npm test` (`vitest run`), `test:watch`, `test:coverage` scripts; `scraper:test`/`shared:test` now point at the same Vitest run instead of raw `tsx` execution of a `node:test` file. E2E (Playwright Test) was explicitly scoped **out** by the human for this session — not installed, no `tests/e2e/` created; §2.6 still describes it as the intended future layer.
  - Converted the two existing fixture-based `node:test` files (`workers/scraper/__tests__/parsers.test.ts`, `workers/shared/lib/__tests__/classifier.test.ts` — previously real assertions, just run via bare `tsx` rather than a real runner) to Vitest's `describe`/`it`/`expect`, each tagged `// @vitest-environment node` since they're pure Node/fs logic with no need for jsdom. Also added the fixture test coverage `parseDateCell` (in `workers/scraper/lib/markdown.ts`) was still missing as of the 2026-07-14 date-parsing entry above — relative-age, yearless-date, Dec/Jan rollover, and the garbage/empty/N/A null-fallthrough cases are now all asserted, not just spot-checked via a throwaway script.
  - New `tests/unit/` directory (per the §1.5 blueprint, which already named this path for app-level unit/component tests) with a `setup.ts` (imports `@testing-library/jest-dom/vitest`; polyfills `ResizeObserver`/`matchMedia`/pointer-capture/`scrollIntoView` on `window`/`Element.prototype`, guarded behind `typeof window !== 'undefined'` so it's a no-op for the node-environment worker tests — these are all things `@base-ui/react`, the headless-UI primitive library `components/ui/*` is built on, needs and jsdom doesn't implement) and two component test files: `tests/unit/components/jobs-table.test.tsx` (5 tests — badge counts, empty state, the `sortForDisplay` generic-link-sinks-to-bottom behavior, optimistic-then-PATCH on the Applied checkbox, Trash tab empty state) and `tests/unit/components/match-results.test.tsx` (5 tests — empty state, ranked row rendering, Run Match success/error paths against a mocked `fetch`, and the Deep Compare expand-and-fetch flow). `next/navigation`'s `useRouter` is mocked in the latter since it throws outside a real Next.js router context. 42 tests total, all passing.
  - New `.github/workflows/ci.yml` (`lint` → `test` → `build`, on push to `main`/`dev` and on PRs) — §2.6 already stated "CI: GitHub Actions runs lint + unit + e2e on every PR" but no such workflow existed; only the two cron workflows (`scraper-cron.yml`, `link-health-cron.yml`) were present. No e2e step since that layer isn't installed. Uses `secrets.NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` for the build step (same two public-safe vars already in §2.3) — **not yet confirmed these secrets exist on the GitHub repo**, human should verify before relying on this workflow actually going green.
  - Verified: `npm test` (42/42 passing), `npm run lint` clean, `npx tsc --noEmit` (root) clean, `npm run build` succeeds. Worker-tsconfig `tsc --noEmit` still shows only the two pre-existing unrelated `import.meta`/CommonJS errors noted in prior entries.
  - **Not done this session**: no tests added for anything under `/app/api/*` (route handlers — would need mocking the Supabase server client, out of scope for this pass), no tests for `add-job-dialog.tsx`, `resume-cell.tsx`, `resume-editor.tsx`, or `job-compare-panel.tsx` in isolation (the latter is exercised indirectly via `match-results.test.tsx`'s Deep Compare test), no coverage thresholds configured/enforced despite `@vitest/coverage-v8` being installed, no fixture tests for `url-quality.ts`/`dead-listing.ts`/the Playwright-fallback path in `html.ts` (still an open gap from the 2026-07-03 entries above). §2.5/§2.6 should be updated by a human to reflect the now-real dependency list and the new `ci.yml`, and §1.5's blueprint already named `tests/unit/` so no change needed there — flagging per Section 0 since this is a dependency-policy-relevant addition even though it was pre-approved in-session.
...
**[2026-07-14] Ran `scraper:dev` then `link-health:dev` ad hoc (human request, "run the scraper and link validator").** Scraper: 3,207 listings attempted across the 4 sources (272 dead-skipped by liveness check), 0 upsert errors. **Found a bug worth someone's attention**: `partitionNewCandidates`'s existing-row dedup lookup (`workers/scraper/index.ts`) hit `TypeError: fetch failed` against Supabase on every chunk, for every source, this entire run — its catch-and-continue behavior (logs `Existing-row lookup failed, treating all as new` and proceeds) meant essentially the whole active corpus was treated as "new" candidates rather than just the real nightly delta, triggering the expensive inline `fetchTextWithTimeout → checkListingLiveness → classifyUrlQuality` pipeline on ~3,200 listings instead of a handful (also visibly stressed the Groq key pool — 3 keys rotated-out mid-run on `simplify-newgrad`). **No data corruption**: the final write path is `upsert(..., { onConflict: 'company,title,url', ignoreDuplicates: true })`, which is a no-op on real conflicts regardless of what the pre-check thought, so existing rows (applied state, resume paths, etc.) were not touched or overwritten. Root cause of the repeated `fetch failed` not investigated this session — worth checking whether it's a transient local/network blip or a recurring Supabase reachability issue, since it'll silently degrade every future scraper run into a full-corpus classify pass the same way if it recurs. Silver lining: post-run the active corpus (3,810 rows) is fully classified — 0 rows with null `url_quality` or null `last_checked_at` (3,212 direct / 521 generic / 77 unknown) — so `link-health`'s backlog is currently empty.

Then ran `link-health:dev`: processed its normal 300-row batch (oldest `last_checked_at` first, since there was no null-backlog left), 19 flagged dead, 19 fetch failures, 0 update errors. Both runs used the real service-role client against the remote Supabase project (not a dry run). Not done this session: no fix attempted for the dedup-lookup `fetch failed` bug, no fixture test added for it.

**[2026-07-14] Sort dashboard by `posted_at` (nulls last); fixed two scraper date-parsing gaps, ad hoc (human request).** `app/dashboard/page.tsx`'s shared `baseQuery()` (feeds all three fetches — internship tab, new_grad tab, trash tab) now orders `.order('posted_at', { ascending: false, nullsFirst: false }).order('discovered_at', { ascending: false }).order('id', { ascending: false })`, i.e. newest-posted-first with unknown-`posted_at` rows sinking below every row with a known date, using `discovered_at`/`id` as tiebreakers. No DB/RLS change — pure query-ordering change. The existing client-side `sortForDisplay` in `components/dashboard/jobs-table.tsx` (sinks `url_quality === 'generic'` rows within a tab) is a stable sort, so it composes fine with the new DB ordering — no change needed there.

While investigating why so many rows would've landed in the "unknown date" bucket, found and fixed two real bugs in how `posted_at` gets populated (the "try harder" half of the ask), both in markdown-table sources under `workers/scraper/sources/`:
- **`speedyapply.ts`**: its date column is headed **"Age"** (relative days-ago, e.g. `"8d"`), not "Date" — the old `dateKey = keys.find(k => k.includes('date'))` never matched it, so `posted_at` was silently `null` for every speedyapply-sourced row (both README.md and NEW_GRAD_USA.md), 100% of the time. Fixed by matching an `'age'`-named column first, falling back to `'date'` for other layouts.
- **`vanshb03.ts`**: its "Date Posted" column uses a yearless short format (e.g. `"Jul 09"`). The old code did `new Date(row[dateKey])` directly — `new Date('Jul 09')` doesn't throw or return `Invalid Date`, it silently parses to **the year 2001**, so every vanshb03 row's `posted_at` was garbage-old (would've sorted dead last under the new nulls-last-but-real-dates-first order, worse than if it had just been `null`).

Fix for both: added `parseDateCell(raw: string): Date | null` to `workers/scraper/lib/markdown.ts`, handling the two known shapes explicitly — `/^\d+d$/i` as days-ago, and a strict `/^[A-Za-z]{3,9}\.?\s+\d{1,2}(st|nd|rd|th)?$/` match (e.g. "Jul 9", "Jul 09") as a yearless date with the current year appended, rolled back one year if that lands more than a day in the future (handles the Dec/Jan boundary). Deliberately narrow regexes rather than a looser "does this look date-ish" check — an earlier draft that appended a year to *any* string without a 4-digit substring caused JS's lenient `Date` parser to silently turn unrelated garbage text (e.g. `"garbage"`) into `Jan 1 <current year>` (verified via `new Date('garbage 2026')` → valid `Thu Jan 01 2026`); the tightened regex makes non-matching cells correctly fall through to `null` instead. Both sources' `rowToListing`/inline row-parsing now call `parseDateCell` instead of raw `new Date(...)`. The two Simplify sources (`simplify-internships.ts`, `simplify-newgrad.ts`) were already correct — they parse a genuine Unix timestamp (`date_posted`) from upstream JSON — and weren't touched.

Verified: `npm run lint` clean; `npx tsc --noEmit -p workers/scraper/tsconfig.json` shows only the two pre-existing unrelated `import.meta`/CommonJS errors noted in earlier entries (`parsers.test.ts`, `text-classifier.ts`); `npm run build` succeeds. `parseDateCell` spot-checked against real values pulled live from both upstream READMEs (`8d`, `0d`, `119d`, `Jul 09`, `Dec 31`) plus edge cases (`garbage`, empty string, ISO date, `N/A`, `-`) via a throwaway Node script (deleted after) — all resolved as expected. **Not done this session**: no fixture/unit test added for `parseDateCell` (repo still has no `tests/` dir or `test` script, a pre-existing gap noted in earlier entries); the fix hasn't been exercised via a real `scraper:dev` run yet, so the *next* scraper run against speedyapply/vanshb03 should be checked to confirm `posted_at` is actually populating now instead of the two failure modes above. Existing rows in the live corpus still carry whatever `posted_at` (`null`, or a bogus year-2001 date for vanshb03 rows) they got from prior runs — this fix only changes what happens on future upserts, it doesn't backfill/correct already-stored rows; worth a one-off backfill or waiting for the nightly cron's natural churn to depend on, human's call.