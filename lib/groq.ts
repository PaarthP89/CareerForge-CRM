...
// is independent (confirmed: each maps to a distinct Groq org), but the
// *aggregate* throughput across all keys is still bounded, so immediately
// hammering the next key on failure just burns through the whole pool in
// seconds instead of pacing against what it can actually sustain. Runs up to
// MAX_KEY_PASSES full loops over the key list, since a single pass may not
// be enough time for the earliest keys to recover.
export async function generateText(prompt: string): Promise<string> {
  ...
  throw lastErr instanceof Error ? lastErr : new Error('All Groq keys failed after retries');
}
